...
export default function Tab() {
  ...
  function handleScroll() {
    ...
    const fActivate =
      pageYOffset !== 0 && pageYOffset >= 0 && pageYOffset < dLocation
    const dActivate =
      pageYOffset !== 0 && pageYOffset >= dLocation && pageYOffset < rLocation
  ...
  return (
    <ProductTab>
      ...
      <ProductTabName
        className={fActivate && 'activate'}
        onClick={() => moveLocation(0)}
      >
        기능
      </ProductTabName>
      <ProductTabName
        className={dActivate && 'activate'}
        onClick={() => moveLocation(1)}
      >
        상세
      </ProductTabName>
      <ProductTabName
        className={rActivate && 'activate'}
        onClick={() => moveLocation(2)}
      >
        리뷰
      </ProductTabName>
    </ProductTab>
